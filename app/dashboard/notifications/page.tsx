    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">{unreadCount > 0 ? `${unreadCount} ${t(lang,'dashboard.unread')}` : t(lang,'dashboard.allCaughtUp')}</p>
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead} className="flex items-center gap-2 text-sm text-[#c9a84c] hover:underline">
      ...
      <div className="space-y-3">
        {notifications.length === 0 ? (
          <div className="card-dark p-12 text-center text-gray-600">
            ...
            <p>{t(lang,'dashboard.noNotifications')}</p>
          </div>
        ) : notifications.map((notif: any) => {
          const { icon: Icon, color } = TYPE_ICONS[notif.type] || TYPE_ICONS.info
          return (
            <div key={notif.id} className={`card-dark p-5 flex gap-4 transition-all ${!notif.isRead ? 'border-[#c9a84c]/20 bg-[#c9a84c]/3' : ''}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${color}`}><Icon size={18} /></div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className={`font-semibold text-sm ${!notif.isRead ? 'text-white' : 'text-gray-300'}`}>{notif.title}</div>
                  {!notif.isRead && <div className="w-2 h-2 rounded-full bg-[#c9a84c] flex-shrink-0 mt-1.5" />}
                </div>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  {notif.link && <Link href={notif.link} className="text-[#c9a84c] text-xs hover:underline">View →</Link>}
                </div>
              </div>
            </div>
          )
