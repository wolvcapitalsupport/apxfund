...
// ═══════════════════════════════════════════════════════════════════════
// 15. PLAN UPGRADE NUDGE — urge user to migrate to a higher plan
// ═══════════════════════════════════════════════════════════════════════
export async function sendPlanUpgradeNudge(
  ...
  const html = wrap(`
    ${heading(`You&apos;re Leaving Money on the Table, ${name}`)}
    ${para(`Your <strong style="color:#c9a84c">${currentPlan}</strong> is performing well — you&apos;ve already earned <strong style="color:#34d399">$${currentProfit.toFixed(2)}</strong> in returns. But there&apos;s a significantly better option waiting for you.`)}

    <div style="background:#0a0a14;border:1px solid #1e1e35;border-radius:12px;padding:20px;margin:20px 0">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap">
        <!-- Current plan -->
        <div style="flex:1;min-width:120px;text-align:center;padding:16px;background:#1a1a2e;border-radius:10px;border:1px solid #2a2a45">
          <div style="font-size:11px;color:#666;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Current Plan</div>
          <div style="font-size:15px;font-weight:700;color:#ffffff;margin-bottom:4px">${currentPlan}</div>
          <div style="font-size:28px;font-weight:900;color:#c9a84c">${currentRoi}%</div>
          <div style="font-size:11px;color:#555">total return</div>
        </div>

        <!-- Arrow -->
        <div style="text-align:center;color:#c9a84c;font-size:24px;font-weight:900">→</div>

        <!-- Next plan -->
        <div style="flex:1;min-width:120px;text-align:center;padding:16px;background:#0d1f0d;border-radius:10px;border:2px solid #34d399">
          <div style="font-size:11px;color:#34d399;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Upgrade To</div>
          <div style="font-size:15px;font-weight:700;color:#ffffff;margin-bottom:4px">${nextPlan}</div>
          <div style="font-size:28px;font-weight:900;color:#34d399">${nextRoi}%</div>
          <div style="font-size:11px;color:#34d399">total return</div>
        </div>
      </div>
    </div>

    ${statusBox('You earn with current plan', `${currentRoi}% total return`, '#c9a84c')}
    ${statusBox('You could earn with upgrade', `${nextRoi}% total return`, '#34d399')}
    ${statusBox('Minimum to unlock upgrade', `$${nextMin.toLocaleString()}`, '#60a5fa')}

    ${divider()}

    ${para(`The difference between staying in <strong style="color:#fff">${currentPlan}</strong> and upgrading to <strong style="color:#34d399">${nextPlan}</strong> is not just percentage points — on larger capital it compounds into significantly higher payouts every single cycle.`)}

    ${para(`Your current capital is already working. Top it up to <strong style="color:#fff">$${nextMin.toLocaleString()}</strong> and migrate to <strong style="color:#34d399">${nextPlan}</strong> before your next contract renews.`)}

    ${btn('Upgrade My Plan Now', `${BASE}/dashboard/plans`)}

    <div style="background:#0a0a14;border:1px solid #1e1e35;border-radius:10px;padding:16px;margin-top:20px">
      <div style="font-size:12px;color:#555;margin-bottom:8px;text-transform:uppercase;letter-spacing:1px">How to upgrade</div>
      ${['Log in to your APXFund dashboard', 'Go to Investment Plans', 'Select your active contract and click <strong style="color:#fff">Migrate to Higher Plan</strong>', 'Top up if needed and confirm — your capital moves instantly'].map((s, i) =>
        `<div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px">
          <div style="width:20px;height:20px;border-radius:50%;background:linear-gradient(135deg,#c9a84c,#e8cc7a);color:#0a0a14;font-size:10px;font-weight:900;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px">${i + 1}</div>
          <span style="font-size:13px;color:#aaa">${s}</span>
        </div>`
      ).join('')}
    </div>

    ${note('You can also enable auto-migrate in your dashboard settings so your capital automatically moves to the next plan when you qualify.')}
  `)
